...
pub(crate) enum PreservedPostSnippet {
    ...
    // selected whitespace lines until write_list knows whether newlines are preserved.
    FileLinesBoundary {
        snippet: String,
...
impl PreservedPostSnippet {
    ...
    fn push_to(&self, result: &mut String, preserve_newline: bool) {
        let PreservedPostSnippet::FileLinesBoundary {
            snippet,
            selected_lines,
        } = self
        else {
            result.push_str(self.snippet());
            return;
        };

        let lines: Vec<_> = snippet.split_inclusive('\n').collect();
        debug_assert_eq!(lines.len(), selected_lines.len());
        let mut i = 0;
        while i < lines.len() {
            let line = lines[i];
            let (line, line_ending) = split_line_ending(line);

            if selected_lines[i] && line.trim().is_empty() {
                if preserve_newline {
                    let mut next = i + 1;
                    while next < lines.len() {
                        let (next_line, _) = split_line_ending(lines[next]);
                        if !selected_lines[next] || !next_line.trim().is_empty() {
                            break;
                        }
                        next += 1;
                    }
...
pub(crate) struct ListItem {
    // Whether the item itself intersects the selected file lines.
    pub(crate) is_selected: bool,
    // Whether the original item was followed by the list separator.
    pub(crate) has_trailing_separator: bool,
    ...
    T: AsRef<ListItem>,
{
    ...
    while let Some((i, item)) = iter.next() {
        ...
        if last && !item.is_selected {
            separate = item.has_trailing_separator;
        }
        ...
        if let Some(ref snippet) = item.preserved_post_snippet {
            snippet.push_to(&mut result, formatting.preserve_newline);
...
fn selected_lines(context: &RewriteContext<'_>, snippet: &str, snippet_lo: BytePos) -> Vec<bool> {
    ...
    snippet
        ...
        .map(|line| {
            ...
            offset += line.len();
            !out_of_file_lines_range!(context, mk_sp(line_lo, line_lo))
        })
        .collect()
}
    ...
    fn next(&mut self) -> Option<Self::Item> {
        self.inner.next().map(|item| {
            ...
            let post_snippet = context
                ...
                .unwrap_or("");
            let has_trailing_separator = post_snippet.find_uncommented(self.separator).is_some();
            ...
            ListItem {
                ...
                } else if self.inner.peek().is_none() && self.leave_last {
